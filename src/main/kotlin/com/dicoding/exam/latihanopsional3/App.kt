...
fun manipulateString(str: String, int: Int): String {
    val numericPart = str.filter { it.isDigit() } // Mengambil bagian angka dari string
    return if (numericPart.isNotEmpty()) {
        // Jika string mengandung angka, kalikan angka pada string dengan integer yang diinput
        val result = numericPart.toInt() * int
        str.replace(numericPart, result.toString())
    } else {
        // Jika string tidak mengandung angka, gabungkan string dengan angka yang diinput
        str + int
    }
}
